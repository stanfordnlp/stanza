...
from stanza.models.common.xlstm import xlstm_stack
...
class NERTagger(nn.Module):
    def __init__(self, args, vocab, emb_matrix=None, foundation_cache=None, bert_model=None, bert_tokenizer=None, force_bert_saved=False, peft_name=None):
        ...
        # recurrent layers
        self.taggerlstm = xlstm_stack(input_size)
        self.reversetaggerlstm = xlstm_stack(input_size)
        ...
        if self.args.get('connect_output_layers'):
            tag_clfs = [nn.Linear(input_size*2, tag_lengths[0])]
            for prev_length, next_length in zip(tag_lengths[:-1], tag_lengths[1:]):
                tag_clfs.append(nn.Linear(input_size*2 + prev_length, next_length))
            self.tag_clfs = nn.ModuleList(tag_clfs)
        else:
            self.tag_clfs = nn.ModuleList([nn.Linear(input_size*2, num_tag) for num_tag in tag_lengths])
    ...
    def forward(self, sentences, wordchars, wordchars_mask, tags, word_orig_idx, sentlens, wordlens, chars, charoffsets, charlens, char_orig_idx):
        ...
        reverse_lstm_inputs = torch.zeros_like(lstm_inputs)
        for i in range(lstm_inputs.size(0)):
            reverse_lstm_inputs[i, 0:sentlens[i], :] = lstm_inputs[i, 0:sentlens[i], :].flip(0)

        #lstm_inputs = PackedSequence(lstm_inputs, inputs[0].batch_sizes)
        lstm_outputs = torch.cat([self.taggerlstm(lstm_inputs), self.reversetaggerlstm(lstm_inputs)], axis=2)
