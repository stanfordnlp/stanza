...
def train_model_one_batch(epoch, batch_idx, model, training_batch, transition_tensors, process_outputs, model_loss_function, similarity_loss_function, oracle, args):
    ...
    if args['similarity_learning_rate'] > 0.0 and epoch <= args['similarity_end_epoch'] and args['transition_scheme'] is TransitionScheme.IN_ORDER:
        ...
        if len(similarity_inputs) > 0:
            ...
            if args['similarity_end_epoch'] != float('inf'):
                similarity_learning_rate = similarity_learning_rate * (args['similarity_end_epoch'] - epoch + 1) / args['similarity_end_epoch']
            similarity_loss = similarity_loss_function(similarity_inputs, similarity_targets) * similarity_learning_rate
