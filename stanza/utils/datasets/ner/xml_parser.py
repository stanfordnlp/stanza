...
for filename in os.listdir(path):
    ...
    while i < len(tokens_):
        ...
        if idx == len(tags_cummulative) or end < tags_cummulative[idx][0] \
          or (start <= tags_cummulative[idx][0] and end < tags_cummulative[idx][1]):
            tagged_tuples.append((t[2], 'O'))

        elif start >= tags_cummulative[idx][0]:
            tagged_tuples.append((t[2], f'B-{tags_cummulative[idx][3]}'))
            while end <= tags_cummulative[idx][1] and i < len(tokens_)-1:
                i += 1
                t = tokens_[i]
                start, end = t[0], t[1]
