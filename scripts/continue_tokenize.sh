...
SAVE_NAME0=${short0}.tok.pkl
SAVE_NAME=${short}2.tok.pkl
...
echo "Running $args..."
CUDA_VISIBLE_DEVICES=$gpu python -m models.tokenizer --${label_type} $labels --txt_file /u/nlp/data/dependency_treebanks/CoNLL18/${treebank}/${short}-ud-train.txt --lang $lang --max_seqlen $seqlen --vocab_file data/tokenize/${short0}_vocab.pkl --save_dir $SAVE_DIR --save_name $SAVE_NAME $train_eval_file --dev_conll_gold $DEV_GOLD --conll_file data/tokenize/${short}.dev.pred2.conllu --shorthand ${short} --load_name $SAVE_NAME0 $args
CUDA_VISIBLE_DEVICES=$gpu python -m models.tokenizer --mode predict $eval_file --lang $lang --mwt_json_file data/tokenize/${short}-ud-train-mwt.json --conll_file data/tokenize/${short}.dev.pred2.conllu --vocab_file data/tokenize/${short0}_vocab.pkl --save_dir $SAVE_DIR --save_name $SAVE_NAME --shorthand $short $args
results=`python utils/conll18_ud_eval.py -v $DEV_GOLD data/tokenize/${short}.dev.pred2.conllu | head -5 | tail -n+3 | awk '{print $7}' | pr --columns 3 -aJT`
