...
echo "Running $args..."
CUDA_VISIBLE_DEVICES=$gpu python -m models.tokenizer --${label_type} $labels --txt_file /u/nlp/data/dependency_treebanks/CoNLL18/${treebank}/${short}-ud-train.txt --lang $lang --max_seqlen $seqlen --vocab_file data/tokenize/${short}_vocab.pkl --mwt_json_file data/tokenize/${short}-ud-train-mwt.json $train_eval_file --dev_conll_gold $DEV_GOLD --conll_file data/tokenize/${short}.dev.pred.conllu --shorthand ${short} $args
CUDA_VISIBLE_DEVICES=$gpu python -m models.tokenizer --mode predict $eval_file --lang $lang --conll_file data/tokenize/${short}.dev.pred.conllu --vocab_file data/tokenize/${short}_vocab.pkl --shorthand $short $args
